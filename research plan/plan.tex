...
So far, it remains unclear how high the prize of explainability in the context of those models.
Like in the case of decision trees, we strove to investigate theoretically-achievable quality guarantees for new models, while we aim to retain sufficiently simple models that enhance explainability.

Next, we plan to investigate explainable clustering in the context of \emph{knowledge-seeking applications}, where a commonly-overlooked problem is that (rule-based) explanations often repeat already known most salient signals, thus failing to convey knowledge.
To address this issue, we wish to research algorithms that directly provide a diverse set of explanations that together provide multiple diverse perspectives and reasons for clusters in datasets, by building on research on diverse rule sets~\cite{}.
Specifically in this context, we further like to explore the importance of features in groups, highlighting informative feature-values that are characteristic, contrastive, or common in groups of the data.
There is a rich body of research on measuring the importance of individual features, such as Shapley values~\cite{} or impurity~\cite{}.
We hypothesize that sets of \emph{interactions between features} are highly relevant reasons behind clusters.
...
The reason for this is clear: there are exponentially many interactions between features and each interaction is in itself a multi-variate and non-linear, thus providing significant algorithmic and statistical challenges.  
We hope to explore novel measures to judge the importance of higher-order feature interactions, leveraging ideas from information theory, statistics, and statistical learning. 
