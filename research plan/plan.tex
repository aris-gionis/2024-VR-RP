...
Traditional cluster-specific feature importance measures include impurity coefficients~\cite{todo}, statistical testing~\cite{todo}, or 
...
A subset of research on \textbf{explainable clustering} methods aims to identify the reasons behind a clustering through a set of simple feature-based decisions that almost mechanistically leads to the cluster. 
Considerable attention has been given to explaining clusters with decision trees, 
in which explanations are sequences of simple yes-no questions based on the data features,
such as \texttt{if} $0 \leq x \leq 2$ \texttt{then} test left branch \texttt{else} test right branch \texttt{end}, that lead to a cluster in a leaf of the decision tree.  
\textcolor{red}{Recent advancements have improved the theoretically attainable worst-case quality~\cite{todo,todo,todo} under axis-aligned decisions or improving interpretability by integrating decision trees directly into the clustering algorithm~\cite{}. 
...
Reaching clusters by means of such an interpretable model has approximation cost, which is called the prize of explainability.
Central to our research is the question of how large the prize to pay becomes, i.e., what is the worst-case-achievable approximation guarantee of expressing a clustering in terms of a specific interpretable machine learning model. 

Although the prize for explaining $k$-means with axis-aligned decision tree might be low~\cite{}, a recent investigation~\cite{} shows that the cost lies in the size of the tree.
Simply put, a precise explanation requires large trees.
The larger the clustering complexity, the larger the explainations, thereby shrinking interpretability to a point where the benefits of having an explanation over analyzing raw data diminishes.
...
by advancing decision trees, exploring rule-based explainations, and by investigating important features for clusters.
...
One concrete goal is study how we can explain clusters with concise decision rule lists~\cite{}, of the form \texttt{if} $\bigwedge_i \llbracket x_i \in [\textnormal{lb}, \textnormal{ub}] \rrbracket$ \texttt{then} cluster$_a$ \texttt{else} proceed to the next rule \texttt{end}. 
...
We aspire to change this by studying the theoretical underpinning, the prize of diversity for explainability clustering, its practical implication on clustering accuracy, and the ability to convey knowledge. 
...
\note{Solution details? Removing this section?}
...
We hope to explore the cost that joining explanations with clustering would inflicts on the clustering performance, computational complexity, and explainability of the results.

% \note{

% Central to our research is the concept of the prize of explainability, as the worst-case achievable approximation guarantee of expressing a clustering in terms of a different interpretable machine learning model.
...
Most related recent research on explainable clustering has focussed on only classical $k$-means
...
In fact, a very recent advancement has discovered that the worst-case prize of explaining quadratic-kernel based clustering with axis-aligned trees is theoretically unbounded~\cite{}. 
...
\textbf{(4)} Going beyond worst-case analysis, what are the \emph{expected} or the \emph{lower bound} to the prize of explainability?; and
\textbf{(5)} Can we judge approximation guarantees considering the geometries of explanations and clusterings?
