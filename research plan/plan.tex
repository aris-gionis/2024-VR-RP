...
represented by both Euclidean vectors and covariate attributes, 
it is immediate clear how to leverage the vectorial information
with one of the numerous clustering methods, such as $k$-means or spectral clustering. 
...
We propose to use the covariate attributes for explaining the clustering. 
In particular, we will study the problem of designing clustering methods
that achieve low cost when considering the vector representation of the data points, 
while the covariates provide an easy and intuitive way to describe each cluster.
